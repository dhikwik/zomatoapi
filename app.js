...
const mongoUrl="mongodb+srv://mango:mango12345@cluster0.km6ui.mongodb.net/zomatoTest?retryWrites=true&w=majority"
...
app.get('/restaurent/:id',(req,res) => {
    ...
    db.collection('zomato').find({state_id:restId}).toArray((err,result) =>{
        if(err) throw err;
        res.send(result)
    })
})*/
...
// mealType
app.get('/menuType',(req,res) => {
    db.collection('menuType').find().toArray((err,result) =>{
        if(err) throw err;
        res.send(result)
    })
})
